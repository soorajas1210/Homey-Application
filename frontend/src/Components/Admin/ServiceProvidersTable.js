...
export default function ServiceProvidersTable() {
  ...
  }, [isAdmin, user, dispatch, navigate]);
      ...
      buttons: [
        {
          label: "Yes",
          onClick: () => {
            dispatch(blockProvider(id, status));
            // navigate("/admin");
          },
        },
        {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = provider.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - provider.length) : 0;
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={provider.length}
            />
            {!loading && !error && (
              <TableBody>
                {stableSort(provider, getComparator(order, orderBy))
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          ...
                          >
                            {user.role === "provider" ? "Block" : "Unblock"}
                          </Button>
                        </TableCell>
                      </TableRow>
                    );
